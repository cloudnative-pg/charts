...
| additionalVolumeMounts | list | `[]` | Array containing extra volume mounts to be added to the operator container, which can be templated. |
| additionalVolumes | list | `[]` | Array containing extra volumes to be added to the operator pod, which can be templated. |
...
| monitoring.podMonitorScheme | string | `""` | Scheme to use when scraping the metrics endpoint. Set it to `https` when TLS is enabled on the operator metrics server through `METRICS_CERT_DIR`. |
| monitoring.podMonitorTLSConfig | object | `{}` | TLS configuration to use when scraping the metrics endpoint, which can be templated. |
